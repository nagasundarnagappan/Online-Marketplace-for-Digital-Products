...
public class JwtUtil {
    ...
    public String generateBToken(Buyer buyer) {

        JwtBuilder builder = Jwts.builder();

        builder.setSubject(buyer.getBuyerId() + "," + buyer.getEmail() + "," + "Buyer").setIssuer("RITHIK RAJ")
    ...
    public String generateSToken(Seller seller) {

        JwtBuilder builder = Jwts.builder();

        builder.setSubject(seller.getSellerId() + "," + seller.getEmail() + "," + "Seller").setIssuer("RITHIK RAJ")
                .setIssuedAt(new Date())
                .setExpiration(new Date(System.currentTimeMillis() + TimeUnit.HOURS.toMillis(5)))
                .signWith(SignatureAlgorithm.HS512, secret.getBytes());
